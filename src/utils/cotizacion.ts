...
export function normalizeCotizacion(cotizacionDetalle: any, cotizacionId?: number): NormalizedCotizacion | null {
  if (!cotizacionDetalle) return null;
...
export function filterSimilares(
  rows: any[],
  cotizacionId: number,
  cotizacionDetalle: any,
  filtrarPorTemporada: boolean,
  ...
  return rows
    .filter(r => {
      ...
      if (filtrarPorTipo && cotizacionDetalle?.TTIPOCOTI) {
        ...
        if (rowTipo !== cotizacionDetalle.TTIPOCOTI) return false;
      }
    ...
    .sort((a, b) => {
      ...
      if (costoA > 0 && costoB === 0) return -1;
      if (costoB > 0 && costoA === 0) return 1;
...
export function buildCotizacionInfo(normCot: NormalizedCotizacion): string {
  let info = `
...
export function buildSimilaresSection(similares: SimilarCotizacion[], needsSimilares: boolean): string {
  if (!needsSimilares) return '';
  ...
  const similaresText = similares.map((s) => {
    ...
    if (s.markup != null) parts.push(`Markup: ${s.markup}%`);
...
export function buildDefaultReply(similares: SimilarCotizacion[]): string {
  if (!similares.length) return 'No hay similares';
