...
import type { ConversationState, MenuSelectionResult, MenuOption } from '../types/index.js';

// Detectar estado de la conversación basado en el mensaje
export function detectConversationState(mensaje: string): ConversationState {
...
export function generateMenuResponse(type: string, cotizacionId?: number): string {
  const menu = CONVERSATIONAL_MENUS[type as keyof typeof CONVERSATIONAL_MENUS];
...
export function generateMainMenuResponse(cotizacionId?: number): string {
  return generateMenuResponse('main', cotizacionId);
}

// Obtener opciones de un menú
export function getMenuOptions(type: string): MenuOption[] {
  const menu = CONVERSATIONAL_MENUS[type as keyof typeof CONVERSATIONAL_MENUS];
...
export function processMenuSelection(selection: string, currentMenu: string = 'main'): MenuSelectionResult {
  ...
  const menu = CONVERSATIONAL_MENUS[currentMenu as keyof typeof CONVERSATIONAL_MENUS];
  ...
  const matchedOption = menu.options.find(opt =>
    opt.label.toLowerCase().includes(lowerSelection) ||
...
export function addNavigationOptionsToResponse(response: any): any {
  return {
    ...response,
    isMenu: true,
