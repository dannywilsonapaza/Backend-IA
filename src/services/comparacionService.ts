...
import type { CandidatoComparacion, KPIsComparacion, ComparacionResult, CotizacionActualInfo, MinutajeComparacion, ComponenteComparacion } from '../types/index.js';
...
export async function compararMinutajes(
  ...
  try {
    ...
    const result: BackendResponse<MinutajeComparacion[]> = await response.json();
    return {
      success: true,
      data: result.data || [],
      warning: result.warning
...
export async function compararComponentes(
  ...
  try {
    ...
    const result: BackendResponse<ComponenteComparacion[]> = await response.json();
    return {
      success: true,
      data: result.data || [],
      warning: result.warning
 ...
 */
export function seleccionarMejorCandidato(candidatos: CandidatoComparacion[]): CandidatoComparacion | null {
