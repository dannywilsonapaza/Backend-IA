...
import type { NormalizedCotizacion, SimilarCotizacion } from '../types/index.js';

// Respuesta fast-path para consultas predefinidas (sin LLM)
export function handlePredefinedQuery(
  queryId: string,
  cotizacionDetalle: NormalizedCotizacion | null,
  similares: SimilarCotizacion[],
  cliente: string,
  temporada: string
): string | null {
  const query = PREDEFINED_QUERIES.find(q => q.id === queryId);
  if (!query?.fastPath) return null;
...
function buildResumenGeneral(normCot: NormalizedCotizacion | null): string {
  ...
  if (precio === 0) result += ` (pendiente de definir)`;
  result += `\n`;
...
function buildPrecioActual(normCot: NormalizedCotizacion | null): string {
  ...
  if (precioActual === 0) result += ` (pendiente de definir)`;
  result += `\n`;
...
function buildCostoActual(normCot: NormalizedCotizacion | null): string {
...
function buildMarkupActual(normCot: NormalizedCotizacion | null): string {
...
function buildInfoCliente(
  normCot: NormalizedCotizacion | null,
  cliente: string,
  temporada: string
): string {
...
function buildDetallesEspecificos(normCot: NormalizedCotizacion | null): string {
...
export function buildForcedDatosResponse(queryId: string, normCot: any, cotizacionId?: number, cliente?: string, temporada?: string): string {
  const nc = normCot || {};
  ...
  switch (queryId) {
    case 'resumen-general': {
      ...
      if (precio === 0) r += ` (pendiente de definir)`;
      r += `\n• Costo: ${fmt.money(costo)}\n`;
      r += `• Markup: ${fmt.pct(markup)}`;
      if (!markup && precio === 0) r += `\n\nℹ️ Nota: Esta cotización aún no tiene precio ni markup definidos`;
      else if (precio === 0) r += `\n\nℹ️ Nota: Esta cotización aún no tiene precio establecido`;
      return r;
    }
    case 'precio-actual': {
