...
import { ejecutarComparacionCompleta, compararKPIs } from '../services/comparacionService.js';
...
export const quoteDetailTool: ToolDefinition = {
  ...
  execute: async (params, ctx): Promise<ToolResult> => {
    ...
    const d0 = Array.isArray((result as any).data?.data) ? (result as any).data.data[0] : (result as any).data?.[0] ?? (result as any).data;
...
async function executeComparisonTool(params: any, ctx: any, grupo: 'ESTILO_CLIENTE' | 'CLIENTE' | 'GLOBAL', intent: string): Promise<ToolResult> {
  const cotizacionId = pickCotizacionId(params);
  if (!cotizacionId) {
    return {
      intent,
      entities: { cotizacionId: null },
      artifacts: [{ type: 'warning', title: 'Falta cotizacionId', data: { message: 'Necesito el ID de la cotización.' } }],
    };
  }
  ...
  if (!comparacion.success || !comparacion.kpis) {
    return {
      intent,
      entities: { cotizacionId },
      artifacts: [{ type: 'warning', title: 'Comparación no disponible', data: { message: comparacion.error || 'No se pudieron calcular los KPIs para esta cotización.' } }],
    };
  }
  ...
  const tableData = [
    { Indicador: 'Precio FOB', Actual: kpis.cotizacionActual.precioFob ?? 'N/D', Anterior: kpis.cotizacionAnterior.precioFob ?? 'N/D', Diferencia: kpis.diferencias.precioFob ?? 'N/D' },
    { Indicador: 'Costo Ponderado', Actual: kpis.cotizacionActual.costoPonderado ?? 'N/D', Anterior: kpis.cotizacionAnterior.costoPonderado ?? 'N/D', Diferencia: kpis.diferencias.costoPonderado ?? 'N/D' },
    { Indicador: 'Markup', Actual: kpis.cotizacionActual.markup ?? 'N/D', Anterior: kpis.cotizacionAnterior.markup ?? 'N/D', Diferencia: kpis.diferencias.markup ?? 'N/D' },
    { Indicador: 'Prendas Est.', Actual: kpis.cotizacionActual.prendasEstimadas ?? 'N/D', Anterior: kpis.cotizacionAnterior.prendasEstimadas ?? 'N/D', Diferencia: kpis.diferencias.prendasEstimadas ?? 'N/D' }
  ];
  ...
  return {
    ...
    entities: { cotizacionId },
    artifacts: [
      { type: 'facts', title: `Resumen de Comparación (${grupo})`, data: {
      ...
      { type: 'table', title: 'Comparación de KPIs', data: { rows: tableData, total: 4 } }
    ],
  };
}
...
export const quoteCompareClientTool: ToolDefinition = {
  ...
  description: 'Compara los KPIs de la cotización actual con otras cotizaciones históricas del MISMO CLIENTE.',
  requiredParams: ['cotizacionId'],
  examples: ['compara kpis con el mismo cliente para 217442', 'busca cotizaciones similares del cliente para la 217442', 'comparación cliente 217442'],
  execute: async (params, ctx) => executeComparisonTool(params, ctx, 'CLIENTE', 'quote.compare.client'),
};

export const quoteCompareStyleTool: ToolDefinition = {
  id: 'quote.compare.style',
  description: 'Compara los KPIs de la cotización actual con otras cotizaciones históricas que compartan EXACTAMENTE EL MISMO ESTILO del cliente.',
  requiredParams: ['cotizacionId'],
  examples: ['compara kpis del mismo estilo para la 217442', 'busca cotizaciones con el mismo estilo de la 217442', 'comparación estilo 217442'],
  execute: async (params, ctx) => executeComparisonTool(params, ctx, 'ESTILO_CLIENTE', 'quote.compare.style'),
};

export const quoteCompareGlobalTool: ToolDefinition = {
  id: 'quote.compare.global',
  description: 'Compara los KPIs de la cotización actual con otras cotizaciones históricas A NIVEL GLOBAL (toda la base de datos).',
  requiredParams: ['cotizacionId'],
  examples: ['compara kpis a nivel global de la 217442', 'búsqueda inteligente global para la 217442', 'comparación global 217442'],
  execute: async (params, ctx) => executeComparisonTool(params, ctx, 'GLOBAL', 'quote.compare.global'),
};

export const quoteCompareTwoTool: ToolDefinition = {
  id: 'quote.compare.two',
  description: 'Compara los KPIs y costos financieros entre DOS cotizaciones ESPECÍFICAS proporcionando ambos IDs.',
  requiredParams: ['cotizacionA', 'cotizacionB'],
  examples: ['compara los costos de la cotizacion 217442 y 214864', 'diferencias kpis entre 217442 y 214864', 'comparar 217442 vs 214864'],
  execute: async (params, ctx) => {
    ...
    if (!Number.isFinite(cotA) || !Number.isFinite(cotB)) {
      return {
        ...
        entities: { cotizacionA: params.cotizacionA, cotizacionB: params.cotizacionB },
        artifacts: [{ type: 'warning', title: 'Faltan IDs', data: { message: 'Necesito los IDs de ambas cotizaciones.' } }],
      };
    }
    ...
    if (!comparacion.success || !comparacion.data) {
      return {
        ...
        artifacts: [{ type: 'warning', title: 'Comparación no disponible', data: { message: comparacion.error || 'No se pudieron comparar los KPIs para estas cotizaciones.' } }],
      };
    }
    ...
    return {
      ...
      entities: { cotizacionA: cotA, cotizacionB: cotB },
      artifacts: [
        { type: 'facts', title: `Comparación Directa de KPIs`, data: {
