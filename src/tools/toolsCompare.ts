...
function colorKey(c: any): string {
...
export const compareColorsTool: ToolDefinition = {
  id: 'compare.colors',
  ...
  execute: async (params, ctx): Promise<ToolResult> => {
    ...
    const aRows = (ra.data ?? []) as any[];
    const bRows = (rb.data ?? []) as any[];

    const aMap = new Map<string, any>();
    for (const c of aRows) aMap.set(colorKey(c), c);
    const bMap = new Map<string, any>();
    for (const c of bRows) bMap.set(colorKey(c), c);
    ...
    for (const [k, v] of aMap.entries()) {
      if (bMap.has(k)) common.push(normalizeColor(v));
      else onlyInA.push(normalizeColor(v));
    }
    for (const [k, v] of bMap.entries()) {
      if (!aMap.has(k)) onlyInB.push(normalizeColor(v));
    }
